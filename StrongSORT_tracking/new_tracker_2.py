...
class Main:
    ...
    def run(self):
        ...
        while True:
            ...
            if hasattr(self.strongsort, 'tracker') and prev_frames is not None and curr_frames is not None:
                self.strongsort.tracker.camera_update(prev_frames, curr_frames)

            frame = camera_data['image']
            outputs, confs = self.perf_track(frame)
            ...
            if len(outputs) > 0:
                for j, (output, conf) in enumerate(zip(outputs, confs)):
                    frame = self.annotation(frame, output, conf, fps)
